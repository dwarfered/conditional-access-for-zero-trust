...
<#
    .SYNOPSIS
        CA Policy: CA207-Internals-AttackSurfaceReduction-AllApps-AnyPlatform-BlockUnknownPlatforms

    .DESCRIPTION
        A CA Policy to block unknown platforms.
...
function Initialize-RequiredPersonaGroups {
    ...
    $prerequisiteGroups = @(
        ...
        'CA-Persona-Internals-AttackSurfaceReduction-Exclusions')
...
$policyName = 'CA207-Internals-AttackSurfaceReduction-AllApps-AnyPlatform-BlockUnknownPlatforms'
...
if (Get-MgIdentityConditionalAccessPolicy -Filter "displayName eq  '$policyName'") {
    ...
    $policy.Conditions.Users.ExcludeGroups = @($personaGroups['CA-BreakGlassAccounts'], $personaGroups['CA-Persona-Internals-AttackSurfaceReduction-Exclusions'])
    $policy.Conditions.Applications.IncludeApplications = 'All'
    $policy.Conditions.ClientAppTypes = 'all'
    $policy.Conditions.Platforms.ExcludePlatforms = @('android', 'iOS', 'windows', 'macOS', 'linux')
